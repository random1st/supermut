//! supermut._core — fast local inference for mutation generation.
//!
//! Design: `Engine` owns the backend + model (loaded once, GPU-offloaded where
//! available). A `LlamaContext` borrows the model, so it is created per call
//! rather than stored. The supermut workload — one shared file prefix, many
//! short continuations — is served by `generate_batch`: the prefix is decoded
//! once into sequence 0, its KV cache is shared with every continuation, and
//! continuations decode in parallel waves (one token per live sequence per
//! `decode` call), so the GPU crunches the whole wave in a single pass.
...
/// How many continuations decode in parallel per wave. Each wave step feeds
/// one token per live sequence into a single `decode`, so the GPU processes
/// the whole wave in one pass instead of one sequence at a time.
...
fn backend() -> PyResult<&'static LlamaBackend> {
    if BACKEND.get().is_none() {
        // `Engine` is an unsendable pyclass, so construction runs under the
        // GIL on one thread — no init race.
        let b = LlamaBackend::init().map_err(rt_err)?;
        let _ = BACKEND.set(b);
    }
...
struct Slot {
    seq: i32,
    pos: i32,
    /// Index of this slot's logits in the most recent decoded batch.
    logits_idx: i32,
...
impl Engine {
    fn new_context(&self, n_seq_max: u32) -> PyResult<LlamaContext<'_>> {
        let params = LlamaContextParams::default()
            .with_n_ctx(NonZeroU32::new(self.n_ctx))
            .with_n_batch(self.n_ctx.max(512))
            .with_n_seq_max(n_seq_max)
            // One shared KV buffer: sequences reference the same prefix cells,
            // so copy_kv_cache_seq marks cells instead of duplicating them.
            .with_kv_unified(true);
        self.model.new_context(self.backend, params).map_err(rt_err)
    }

    /// Decode `tokens` into `seq` starting at `start_pos`; request logits only
    /// for the last token. Returns the position after the last token.
    fn decode_tokens(
        ...
        for chunk in tokens.chunks(n_batch) {
            ...
            for (i, tok) in chunk.iter().enumerate() {
                ...
                batch.add(*tok, pos, &[seq], is_last).map_err(rt_err)?;
                pos += 1;
            }
            ctx.decode(batch).map_err(rt_err)?;
        }
        Ok(pos)
    }
    ...
    /// Run one wave of continuations to completion. `slots` already have their
    /// continuation tails decoded and `logits_idx` pointing into the last batch.
    fn wave_sample_loop(
        &self,
        ctx: &mut LlamaContext<'_>,
        batch: &mut LlamaBatch,
        ...
        for _ in 0..max_tokens {
            // Sample one token per live slot, then decode them all in one pass.
            batch.clear();
            ...
            for slot in slots.iter_mut() {
                ...
                batch
                    .add(tok, slot.pos, &[slot.seq], true)
                    .map_err(rt_err)?;
                slot.pos += 1;
...
impl Engine {
    ...
    fn py_new(
        ...
        if !model_path.exists() {
            return Err(rt_err(format!("model file not found: {model_path:?}")));
        }
        let backend = backend()?;
        let model_params = LlamaModelParams::default().with_n_gpu_layers(n_gpu_layers);
        let model =
            LlamaModel::load_from_file(backend, &model_path, &model_params).map_err(rt_err)?;
    ...
    /// in parallel waves of up to `WAVE_SIZE` sequences. This is the supermut
    /// hot path (one source file -> many mutant completions).
    ...
    fn generate_batch(
        ...
        if prefix_len as u32 + max_tokens as u32 >= self.n_ctx {
            return Err(rt_err(format!(
                "prefix ({prefix_len} tokens) + max_tokens ({max_tokens}) exceeds n_ctx ({})",
                self.n_ctx
            )));
        }
        let last_prefix_tok = *prefix_tokens.last().ok_or_else(|| rt_err("empty prefix"))?;

        // Size the context for the actual batch: a single generation must not
        // pay for WAVE_SIZE sequence streams.
        let wave_size = continuations.len().clamp(1, WAVE_SIZE);
        let mut ctx = self.new_context(wave_size as u32 + 1)?;
        let mut batch = LlamaBatch::new(ctx.n_batch() as usize, wave_size as i32 + 1);

        // Decode the shared prefix once, into SEQ_PREFIX.
        let after_prefix =
        ...
        for (wave_no, wave) in continuations.chunks(wave_size).enumerate() {
            // Seed each slot's sequence with the shared prefix KV cache.
            let mut slots = Vec::with_capacity(wave.len());
            for (j, cont) in wave.iter().enumerate() {
                let seq = j as i32 + 1;
                ...
                slots.push(Slot {
                    ...
                    sampler: SamplerSpec {
                        ...
                        top_p,
                        min_p,
                        seed: seed.wrapping_add(global_i as u32),
                    }
                    .build(),
                ...
                let _ = cont; // decoded below, once all slots hold the prefix
            }
            ...
            for (slot, cont) in slots.iter_mut().zip(wave.iter()) {
                if cont.is_empty() {
                    ...
                    let cont_tokens = self
                        .model
                        .str_to_token(cont, AddBos::Never)
                        .map_err(rt_err)?;
                    if prefix_len + cont_tokens.len() + max_tokens >= self.n_ctx as usize {
                        return Err(rt_err(format!(
                            "prefix + continuation ({} tokens) + max_tokens ({max_tokens}) exceeds n_ctx ({})",
            ...
            ctx.decode(&mut batch).map_err(rt_err)?;

            self.wave_sample_loop(&mut ctx, &mut batch, &mut slots, max_tokens, &stop)?;
            results.extend(slots.into_iter().map(|s| s.out));
        }
        Ok(results)
    }

    /// Count tokens for a string (prefix budgeting).
